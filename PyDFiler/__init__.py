...
class PyDFiler:
    ...
    def generate_PDF_from_dashboard(self, dashboard_uuid, output_folder, range_time_from='now-1d', range_time_to='now', output_pdf=Config.wkhtmltopdf.output_pdf, print_title=False):
        ...
        columns, rows = self.calculate_collage_dimensions(number_of_images)

        collage_width_final = int(columns * 300)
        collage_height_final = int(rows * 120)

        collage_output_path = output_folder / 'collage.png'
        collage_path = self.create_collage_from_dict(width=collage_width_final,
                                                     height=collage_height_final,
                                                     images_dict=cropped_images_rgb,
                                                     collage_output_path=collage_output_path,
                                                     col=columns,
                                                     row=rows)
    ...
    def generate_html_from_template(self, output_dir, template_path=Config.Default.Template.path, replace_dict=None):
    ...
    def calculate_collage_dimensions(self, number_of_images:int):
        columns = math.sqrt(number_of_images)
        lines = math.ceil(number_of_images / columns)

        return round(columns), lines
